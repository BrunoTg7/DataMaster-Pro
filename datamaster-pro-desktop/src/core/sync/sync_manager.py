...
class SyncManager:
    ...
    def sync_now(self) -> Dict:
        ...
        """
        if self.is_syncing: return {"success": False, "error": "Em andamento"}
        if not self.check_connection(): return {"success": False, "error": "Sem conexão", "offline": True}
        ...
        try:
            ...
            if access_token: supabase.postgrest.auth(access_token)

            pending = self.get_pending_items()
            if not pending: return {"success": True, "synced": 0}
            ...
            for item in pending:
                try:
                    ...
                    supabase.table("execucoes").insert(mapped_data).execute()
            ...
            # Trigger background sync cloud function
            if results["synced"] > 0:
...
class ExecutionTracker:
    ...
    def _notify_user(self, tool_name: str, rows: int, hours: float):
        ...
        if not user_data or not user_data.get("notificacoes_desktop", True): return
        try:
            from plyer import notification
            notification.notify(title=f"✅ {tool_name.capitalize()} Concluído!",
                                message=f"Processadas {rows} linhas.\nPoupado: {hours:.1f}h.",
                                app_name="DataMaster Pro", timeout=5)
        except: pass
    ...
    def get_current_cycle_start(self, user_created_at: str) -> datetime:
        ...
        except: return datetime.now() - timedelta(days=30)
    ...
    def get_user_stats(self, user_id: str, start_date: datetime = None) -> Dict:
        ...
        if result is None:
            ...
            result = {"total_lines": total_lines, "total_hours": total_hours, "total_executions": total_execs, "by_tool": stats_by_tool}
