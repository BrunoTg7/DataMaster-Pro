...
class TaskStorage:
    ...
    def _row_to_dict(self, row) -> dict:
        return {
            ...
            "progress_percent": row[3], "progress_message": row[4],
            "input_params": row[5], "output_path": row[6], "log_text": row[7],
            "rows_processed": row[8], "hours_saved": row[9],
            "created_at": row[10], "updated_at": row[11], "completed_at": row[12],
            "user_id": row[13], "error_message": row[14], "tool_display_name": row[15]
        }
    ...
    def save_task(self, task_data: dict):
        ...
        """, (
            ...
            task_data.get("progress_percent", 0), task_data.get("progress_message", ""),
            task_data.get("input_params", ""), task_data.get("output_path", ""),
            task_data.get("log_text", ""), task_data.get("rows_processed", 0),
            task_data.get("hours_saved", 0), task_data.get("created_at", ""),
            task_data.get("updated_at", ""), task_data.get("completed_at"),
            task_data.get("user_id", ""), task_data.get("error_message", "")
        ))
        conn.commit()
        conn.close()
    ...
    def get_running_task_by_tool(self, tool_name: str) -> Optional[dict]:
        conn = self._get_conn()
        cursor = conn.cursor()
        cursor.execute(f"SELECT id, tool_name, status FROM tasks WHERE tool_name = ? AND status IN ('pending', 'running')", (tool_name,))
        row = cursor.fetchone()
        conn.close()
    ...
    def get_last_task_by_tool(self, tool_name: str, exclude_failed: bool = True) -> Optional[dict]:
        ...
        cursor = conn.cursor()
        if exclude_failed:
            cursor.execute(f"SELECT {self._COLUMNS} FROM tasks WHERE tool_name = ? AND status NOT IN ('failed', 'interrupted') ORDER BY created_at DESC LIMIT 1", (tool_name,))
        else:
            cursor.execute(f"SELECT {self._COLUMNS} FROM tasks WHERE tool_name = ? ORDER BY created_at DESC LIMIT 1", (tool_name,))
        row = cursor.fetchone()
        conn.close()
        return self._row_to_dict(row) if row else None
